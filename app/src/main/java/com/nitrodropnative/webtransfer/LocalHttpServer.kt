...
internal class LocalHttpServer(
    ...
    private val downloadedFileIds = ConcurrentHashMap.newKeySet<String>()
    private val serverSessionId = "s" + System.currentTimeMillis().toString(36)
        ...
        socket.use { client ->
            ...
            try {
                when {
                    ...
                    request.method == "POST" && request.path == "/api/downloaded/reset" -> resetDownloadedMarks(request, output)
    ...
    private fun serveFiles(request: HttpRequest, output: OutputStream) {
            ...
            registry.snapshot().forEach { file ->
                put(
                    JSONObject()
                        ...
                        .put("downloaded", downloadedFileIds.contains(file.id))
                )
            }
        }
        HttpResponseWriter.json(
            ...
            JSONObject()
                .put("files", items)
                .put("sessionId", serverSessionId)
                .put("downloadedCount", downloadedFileIds.size)
    ...
    private fun resetDownloadedMarks(request: HttpRequest, output: OutputStream) {
        if (!authorized(request)) {
            safeJson(output, "401 Unauthorized", JSONObject().put("error", "Password required").toString())
            return
        }
        ...
        HttpResponseWriter.json(
            ...
            JSONObject()
                .put("ok", true)
                .put("downloadedCount", 0)
                .put("updatedAt", System.currentTimeMillis())
                .toString()
        )
        onStats(baseStats(WebTransferState.Running, true, advertisedDeviceIp, request.remoteAddress).copy(message = "Download marks reset"))
    }
    ...
    private suspend fun streamFile(request: HttpRequest, output: OutputStream, selected: WebSharedFile) {
        ...
        try {
            ...
            if (start == 0L && sent >= bytesToSend) {
                downloadedFileIds.add(selected.id)
            }
